...
function SectorGame({ difficulty, onResolve }: MiniProps) {
  ...
  function pick(sector: number) {
    ...
    if (current >= total) onResolve(8, "Vuelta limpia, todos los sectores conectados");
  ...
  return (
    <Shell title="Sectores">
      ...
      <p>Toca los sectores en orden. La dificultad sube agregando sectores.</p>
      ...
      <div className="mini-status"><span>Siguiente S{current}</span><span>Total {total}</span></div>
    </Shell>
  );
}
